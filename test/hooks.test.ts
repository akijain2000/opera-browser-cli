...
describe("computeHookUpdate", () => {
  it("installs hook when settings have no hooks", () => {
    ...
    expect(hookCmd).toContain("opera-browser-cli");
  });
  ...
  it("installs hook alongside existing hooks", () => {
    ...
    expect(str).toContain("opera-browser-cli");
  });
        ...
        SessionStart: [
          {
            ...
            hooks: [
              {
                type: "command" as const,
                command: "/usr/bin/opera-browser-cli",
                timeout: 10,
              },
            ],
          },
  ...
  it("repairs hook when executable path changed", () => {
        ...
        SessionStart: [
          {
            ...
            hooks: [
              {
                type: "command" as const,
                command: "/old/path/opera-browser-cli",
                timeout: 10,
              },
            ],
          },
    ...
    const [updated, changed] = computeHookUpdate(
      settings,
      "/new/path/opera-browser-cli",
    );
    expect(changed).toBe(true);
    const str = JSON.stringify(updated);
    expect(str).toContain("/new/path/opera-browser-cli");
  ...
  it("repairs hooks regardless of whether the exec path is production-eligible", () => {
        ...
        SessionStart: [
          {
            ...
            hooks: [
              {
                type: "command" as const,
                command: "/usr/local/bin/opera-browser-cli",
                timeout: 10,
              },
            ],
          },
    ...
    const [updated, changed] = computeHookUpdate(
      settings,
      "/Users/kunchen/.airlock/worktrees/bf2b16b1f6b6/pool-3/bin/opera-browser-cli.ts",
    );
    expect(changed).toBe(true);
...
describe("shouldInstallHooksForExecPath", () => {
  ...
  it("accepts packaged dist entrypoints", () => {
    expect(
      shouldInstallHooksForExecPath(
        "/Users/kunchen/github/kunchenguid/opera-cli/dist/bin/opera-browser-cli.js",
      ),
    ).toBe(true);
  });
});
