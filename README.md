...
help[1]:
  Run `opera-cli click @1` to click the "More information..." link
...
```
┌───────────────────────┐
│  opera-cli  │  CLI — parse args, format output
└──────────┬────────────┘
           │ HTTP (localhost:9224)
           ▼
┌───────────────────────┐
...
- **Auto-lifecycle** — the bridge starts on first command and writes a PID file to `~/.opera-cli/bridge.pid`
...
opera-cli eval "(() => { const rows = [...document.querySelectorAll('tr')]; return rows.map((row) => row.textContent) })()"
```
