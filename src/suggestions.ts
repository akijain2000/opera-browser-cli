...
export function getSuggestions(ctx: SuggestionContext): string[] {
  ...
  if (ctx.command === "wait" || ctx.command === "eval") {
    return ["Run `opera-cli snapshot` to see current page state"];
  }
  ...
  if (ctx.command === "fill") {
    ...
    if (submitBtn) {
      lines.push(
        `Run \`opera-cli click @${submitBtn.ref}\` to click "${submitBtn.label}"`,
      );
    } else {
      lines.push("Run `opera-cli press Enter` to submit the form");
    }
  }
  ...
  if (inputs.length > 0 && ctx.command !== "fill") {
    ...
    lines.push(
      `Run \`opera-cli fill @${inp.ref} "text"\` to fill ${label}`,
    );
  }
  ...
  if (buttons.length > 0) {
    ...
    if (btn && !lines.some((l) => l.includes(`@${btn.ref}`))) {
      ...
      lines.push(
        `Run \`opera-cli click @${btn.ref}\` to click the ${label}button`,
      );
    }
  }
  ...
  if (links.length > 0) {
    const link = links[0];
    lines.push(
      `Run \`opera-cli click @${link.ref}\` to click the "${link.label}" link`,
    );
  }

  // Suggest scrolling if page has many elements
  if (refs.length > 5) {
    lines.push("Run `opera-cli scroll down` to scroll down");
  }

  // Teach eval syntax — use IIFE for multi-statement logic
  lines.push(
    'Use `opera-cli eval <expr>` for JS expressions. For multi-statement code, wrap in an IIFE: `eval "(() => { ...; return result })()"`',
  );
