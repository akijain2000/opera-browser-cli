# opera-cli — Claude guidance

## Repository overview

`opera-cli` is a CLI tool and HTTP bridge that connects terminal users (and AI agents)
...
| File | Role |
|---|---|
| `src/cli.ts` | Command parsing and dispatch (`opera-cli <command>`) |
...
| `bin/opera-cli-bridge.js` | Bridge binary entrypoint (calls `runBridge`) |
...
All HTTP requests from `opera-cli` processes share that single MCP connection.
